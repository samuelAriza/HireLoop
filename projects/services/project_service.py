...
class ProjectService:
    def __init__(self, project_repo: ProjectRepository, assignment_repo: ProjectAssignmentRepository, application_repo: ProjectApplicationRepository):
        self.project_repo = project_repo
        self.assignment_repo = assignment_repo
        self.application_repo = application_repo
        
    def apply_to_project(self, project_id, freelancer_id, cover_letter, proposed_payment):
        project = self.get_project(project_id)

        existing = self.application_repo.list_by_project(project.id).filter(
    ...
    def register_interest(self, project_id, freelancer_id, role=""):
        ...
        project = self.get_project(project_id)
        # evita duplicados
        existing = self.assignment_repo.list_by_project(project.id).filter(freelancer_id=freelancer_id).first()
        if existing:
            raise ValueError("Freelancer already registered or assigned to this project.")
