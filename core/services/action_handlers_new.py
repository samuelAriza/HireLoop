# filepath: /home/samargo/Documents/universidad_/software_architecture/hireloop_project/core/services/action_handlers.py
...
class ProfileUpdateHandler(ActionHandler):
    ...
    def handle(self, request: HttpRequest, **kwargs) -> HttpResponse:
        ...
        if form.is_valid():
            try:
                updated_profile = service_methods[profile_type](request.user, form.cleaned_data)
                if updated_profile:
    ...
    def handle(self, request: HttpRequest, **kwargs) -> HttpResponse:
        ...
        try:
            ...
            if portfolio_item.freelancer_profile.user != request.user:
                messages.error(request, 'You do not have permission to edit this portfolio item.')
                return redirect('core:profile_detail')
...
class PortfolioDeleteHandler(ActionHandler):
    ...
    def handle(self, request: HttpRequest, **kwargs) -> HttpResponse:
        ...
        try:
            ...
            # Check ownership
            if portfolio_item.freelancer_profile.user != request.user:
                messages.error(request, 'You do not have permission to delete this portfolio item.')
                return redirect('core:profile_detail')
